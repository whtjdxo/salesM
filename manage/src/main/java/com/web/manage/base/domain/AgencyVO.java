package com.web.manage.base.domain;

import lombok.Data;

@Data
public class AgencyVO {
    private String agency_no;
    private String agency_name;
    private String biz_no;
    private String law_no;
    private String uptae;
    private String upjong;
    private String email;
    private String cont_date;
    private String corp_zip_no;
    private String corp_addr;
    private String corp_addr_dtl;
    private String ceo_id;
    private String ceo_regno;
    private String ceo_name;
    private String ceo_zip_no;
    private String ceo_addr;
    private String ceo_addr_dtl;
    private String corp_tel_no;
    private String ceo_tel_no;
    private String assure_amt;
    private String insure_amt;
    private String ass_sdate;
    private String ass_edate;
    private String remark;
    private String svc_stat;
    private String svc_fee_rate;
    private String crd_fee_rate;
    private String loan_fee_rate;
    private String bank_cd;
    private String account_no;
    private String depositor;
    private String yubo_limit;
    private String yubo_rate;
    private String tot_yubo_amt;
    private String use_yn;
    private String ent_user_id;
    private String ent_dttm;
    private String upt_user_id;
    private String upt_dttm;
}
